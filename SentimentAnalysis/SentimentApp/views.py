...
def dtrf(request):
    if request.method == 'GET':
        ...
        rfc.fit(X, Y)
        predict = rfc.predict(tfidf_X_test)
        rf_acc = (accuracy_score(tfidf_y_test,predict)) * 100
        end = time.time()
        rf_time = end - start
        ...
        dt.fit(X, Y)
        predict = dt.predict(tfidf_X_test)
        dt_acc = (accuracy_score(tfidf_y_test,predict)) * 100
        end = time.time()
        dt_time = end - start

        output = 'Random Forest Accuracy : '+str(rf_acc)+" Random Forest Execution Time : "+str(rf_time)+"<br/>"
        output+='Decision Tree Accuracy : '+str(dt_acc)+" Decision Tree Execution Time : "+str(dt_time)+"<br/>"

        height = [rf_acc, dt_acc, rf_time, dt_time]
        bars = ('Random Forest Accuracy','Decision Tree Accuracy','Random Forest Time','Decision Tree Time')
        y_pos = np.arange(len(bars))
        plt.bar(y_pos, height)
        plt.xticks(y_pos, bars)
        plt.show()
